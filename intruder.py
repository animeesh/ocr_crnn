...
def draw_boxes(img, bbox, object_id, identities=None, offset=(0, 0)):
    ...
    for i, box in enumerate(bbox):
        ...
        # print("LEN_DATA_DEQUR", data_deque[id])

        if len(data_deque[id]) % 3 ==2: #len(data_deque[id]) >= 2:

            # distanceInPixels = math.sqrt(math.pow(data_deque[id][1][0] - data_deque[id][0][0], 2) + math.pow(data_deque[id][1][1] - data_deque[id][0][1], 2))

            # # calculate the time in hours
            # timeInSeconds = len(data_deque[id])
            # # timeInSeconds = abs(t)
            # timeInHours = timeInSeconds / (60 * 60)
            # # calculate distance in kilometers and append the
            # # calculated speed to the list
            # distanceInMeters = distanceInPixels * 5 # meterPerPixel
            # distanceInKM = distanceInMeters / 1000
            # estimatedSpeeds.append(distanceInKM / timeInHours)
            # obj_speed = distanceInKM / timeInHours

            # print("data_deque[id][i]", data_deque[id][1])
            # print("data_deque[id][i-1]", data_deque[id][0])

            obj_speed = estimateSpeed(data_deque[id][1], data_deque[id][0])

            # obj_speed = obj_speed * calilbration_constant * (height - y1)
            
            # speed = np.round(1 * obj_speed,2)
            text_speed = "{} kmh".format(int(obj_speed))
            #if id not in speed_queue:  
            speed_queue[id] = text_speed

        try:
            pass
            #if x1 > 200 and center[1] > (height//2):
                #label = label + " " + str(speed_queue[id])
            # cv2.putText(img, speed_queue[id], (x2, y2), cv2.FONT_HERSHEY_PLAIN, 1, (255,255,255), 2)
        except :
            pass
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if det is not None and len(det):
                ...
                if len(outputs) > 0:
                    ...
                    cv2.putText(im0, 'Intruder Detected', (670, 35), 0, 1, [225, 255, 255], thickness=2, lineType=cv2.LINE_AA)##
